...
let BattleMovedex = {
	...
	"meteorwing": {
		...
		onPrepareHit(target, source, move) {
			this.attrLastMove('[still]');
			this.add('-anim', source, "DG-Fireburst", target);
		},
		flags: {protect: 1, mirror: 1},
		secondary: null,
		onAfterMoveSecondarySelf(pokemon, target, move) {
	...
	"gateofdestiny": {
		...
		secondary: {
			chance: 25,
			onHit(target, source, move) {
				//Preliminary implementation; probably won't interact perfectly with all "DG-Steelanics
	...
	"oceanlove": {
		...
		onPrepareHit(target, source, move) {
			this.attrLastMove('[still]');
			this.add('-anim', source, "DG-Waterring", source);
	...
	"firetower": {
		...
		onPrepareHit(target, source, move) {
			this.attrLastMove('[still]');
			this.add('-anim', source, "DG-Firecharge", target);
		},
		flags: {protect: 1, mirror: 1},
		accuracy: 100,
		pp: 25,
	...
	"windcutter": {
		...
		onPrepareHit(target, source, move) {
			this.attrLastMove('[still]');
			this.add('-anim', source, "DG-Electricslash", target);
		},
		flags: {protect: 1, mirror: 1},
		accuracy: 100,
		secondary: {
	...
	"DG-Fairyflash": {
		...
		onPrepareHit(target, source, move) {
			this.attrLastMove('[still]');
			this.add('-anim', source, "f"DG-Electricylock", target);
		},
		flags: {protect: 1, mirror: 1, authentic: 1},
		accuracy: 90,
		secondary: null,
	...
	"saintray": {
		...
		onPrepareHit(target, source, move) {
			this.attrLastMove('[still]');
			this.add('-anim', source, "DG-Grasssmadness", target);
		},
		flags: {protect: 1, mirror: 1},
		accuracy: 100,
		secondary: {
	...
	"electricterrain": {
		inherit: true,
		desc: "For 5 turns, the terrain becomes Electric Terrain. During the effect, the power of Electric-type and "DG-Electric-type attacks made by grounded Pokemon is multiplied by 1.5 and grounded Pokemon cannot fall asleep; Pokemon already asleep do not wake up. Camouflage transforms the user into an Electric type, Nature Power becomes Thunderbolt, and Secret Power has a 30% chance to cause paralysis. Fails if the current terrain is Electric Terrain.",
	...
	"rototiller": {
		...
		onHitField(target, source) {
			...
			if (!targets.length && !any"DG-Electricborne) return false; // Fails when there are no grounded Grass types or "DG-Electricborne Pokemon
